	...
	public class ClayContainer : MonoBehaviour{
		...
		static public void initGlobalData(){
			...
			ClayContainer.reassignContainerIds();
				...
				// we can count on the globalInit to always be the firtst function to be called
        		for(int i = 0; i < ClayContainer.containersInScene.Count; ++i){
        			ClayContainer.containersInScene[i].init();
        		}
        	#endif
		...
		*/
		...
		/* Clayxels will render internal data to a render texture 1024 x 512 unless you override it here.
		...
		public static void setOutputRenderTextureSize(int width, int height){
			ClayContainer.microvoxelRTSizeOverridden = true;
		...
		*/
		...
		public static void pickingMicrovoxel(Camera camera, float mousePosX, float mousePosY, out int pickedContainerId, out int pickedClayObjectId, bool invertVerticalMouseCoords = false){
			...
			RenderTexture.active = ClayContainer.mvRenderTexture4;
			ClayContainer.pickingTextureResult.ReadPixels(ClayContainer.pickingRect, 0, 0);
			ClayContainer.pickingTextureResult.Apply();
			...
			pickedContainerId = (int)((pickCol3.r * 255) + (pickCol3.g * 255.0f) * 256.0f + (pickCol3.b * 255.0f) * 256.0f * 256.0f) - 1;
			...
			RenderTexture.active = ClayContainer.mvRenderTexture3;
			ClayContainer.pickingTextureResult.ReadPixels(ClayContainer.pickingRect, 0, 0);
			ClayContainer.pickingTextureResult.Apply();
		...
		public static List<ClayContainer> getAllContainersInScene(){
			return ClayContainer.containersInScene;
		}
		...
		static List<ClayContainer> containersInScene = new List<ClayContainer>();
		...
		static bool microvoxelRTSizeOverridden = false;
		...
		static void initMicroVoxelGlobal(){
			...
			*/
			...
			ClayContainer.mvRenderTexture1 = new RenderTexture(resX, resY, 32, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
        	...
        	ClayContainer.mvRenderBuffers = new RenderTargetIdentifier[4];
        	ClayContainer.mvRenderBuffers[0] = new RenderTargetIdentifier(ClayContainer.mvRenderTexture1);
        	ClayContainer.mvRenderBuffers[1] = new RenderTargetIdentifier(ClayContainer.mvRenderTexture2);
        	ClayContainer.mvRenderBuffers[2] = new RenderTargetIdentifier(ClayContainer.mvRenderTexture3);
        	ClayContainer.mvRenderBuffers[3] = new RenderTargetIdentifier(ClayContainer.mvRenderTexture4);
		...
		void initMicroVoxelBuffer(){
        	this.materialProperties.SetTexture("fillerNormalTex", ClayContainer.mvRenderTexture1, RenderTextureSubElement.Color);
			this.materialProperties.SetTexture("fillerGridDataTex", ClayContainer.mvRenderTexture2, RenderTextureSubElement.Color);
			this.materialProperties.SetTexture("fillerColorTex", ClayContainer.mvRenderTexture3, RenderTextureSubElement.Color);
			this.materialProperties.SetTexture("fillerIdTex", ClayContainer.mvRenderTexture4, RenderTextureSubElement.Color);
		...
		void drawClayxelsMicroVoxel(ClayContainer instance){
			...
			this.materialProperties.SetMatrix("objectMatrix", instance.transform.localToWorldMatrix);
			this.materialProperties.SetMatrix("objectMatrixInv", instance.transform.worldToLocalMatrix);
			this.materialProperties.SetInt("containerId", instance.containerId);
		...
		public static RenderBuffer getMicroVoxelDepthBuffer(){
			return ClayContainer.mvRenderTexture1.depthBuffer;
		}

		public static void drawMicroVoxelPrePass(CommandBuffer cmd){
			for(int i = 0; i < ClayContainer.containersInScene.Count; ++i){
				ClayContainer.containersInScene[i].drawMicroVoxelsPass(cmd);
			}
		}

		void drawMicroVoxelsPass(CommandBuffer cmd){
			ClayContainer instance = this;

			if(this.instanceOf != null){
				instance = this.instanceOf;
			}

			if(instance.renderMode != ClayContainer.RenderModes.microVoxel || instance.needsInit || instance.frozen){
				return;
			}
			
			instance.renderBounds.center = this.transform.position;

			instance.materialProperties.SetFloat("chunkSize", (float)instance.chunkSize);
			instance.materialProperties.SetBuffer("chunksCenter", instance.chunksCenterBuffer);
			instance.materialProperties.SetBuffer("boundingBox", instance.boundingBoxBuffer);
			instance.materialProperties.SetBuffer("pointCloudDataMip3", instance.pointCloudDataMip3Buffer);
			instance.materialProperties.SetBuffer("gridPointersMip2", instance.gridPointersMip2Buffer);
			instance.materialProperties.SetBuffer("gridPointersMip3", instance.gridPointersMip3Buffer);
			instance.materialProperties.SetBuffer("chunkIdOffset", instance.chunkIdOffsetBuffer);
			instance.materialProperties.SetMatrix("objectMatrix", this.transform.localToWorldMatrix);
			instance.materialProperties.SetMatrix("objectMatrixInv", this.transform.worldToLocalMatrix);
			instance.materialProperties.SetInt("containerId", this.containerId);
			instance.materialProperties.SetFloat("bufferSizeReduceFactor", ClayContainer.bufferSizeReduceFactor);

			if(instance.memoryOptimized){
				instance.materialProperties.SetInt("memoryOptimized", 1);
			}
			else{
				instance.materialProperties.SetInt("memoryOptimized", 0);
			}

			instance.materialProperties.SetTexture("_MainTex", instance.material.GetTexture("_MainTex"));
			instance.materialProperties.SetFloat("_backFillDark", instance.material.GetFloat("_backFillDark"));
			instance.materialProperties.SetFloat("_splatSizeMult", instance.material.GetFloat("_splatSizeMult"));
			instance.materialProperties.SetFloat("_roughSize", instance.material.GetFloat("_roughSize"));
			instance.materialProperties.SetFloat("_alphaCutout", instance.material.GetFloat("_alphaCutout"));
			instance.materialProperties.SetFloat("_backFillAlpha", instance.material.GetFloat("_backFillAlpha"));
			instance.materialProperties.SetFloat("_roughPos", instance.material.GetFloat("_roughPos"));
			instance.materialProperties.SetFloat("_roughTwist", instance.material.GetFloat("_roughTwist"));
			instance.materialProperties.SetFloat("_roughOrientX", instance.material.GetFloat("_roughOrientX"));
			instance.materialProperties.SetFloat("_roughOrientY", instance.material.GetFloat("_roughOrientY"));
			instance.materialProperties.SetFloat("_roughOrientZ", instance.material.GetFloat("_roughOrientZ"));
			instance.materialProperties.SetFloat("_splatBillboard", instance.material.GetFloat("_splatBillboard"));
			instance.materialProperties.SetFloat("_roughColor", instance.material.GetFloat("_roughColor"));
			...
			cmd.DrawMeshInstancedIndirect(
				...
				passId, 
				instance.volumetricDrawBuffer, 0, instance.materialProperties);
		}
		...
		public static void reassignContainerIds(){
			int previousCount = ClayContainer.containersInScene.Count;

			// lets never destroy containersInScene, in case we needs to hold to the pointer externally
		...
		void updateInstance(){
			...
			if(this.instanceOf.renderMode == ClayContainer.RenderModes.polySplat){
		...
		static void mouseClickMicrovoxel(int mouseX, int mouseY){
			...
			if(pickedContainerId > -1 && pickedContainerId < ClayContainer.containersInScene.Count){
				...
				if(container.editingThisContainer && pickedClayObjectId > -1){
