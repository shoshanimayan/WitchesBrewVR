...
namespace Clayxels{
	...
	public class ClayxelInspector : Editor{
		...
		public override void OnInspectorGUI(){
			...
			EditorGUILayout.Space();
				...
				if(ClayContainer.getRenderPipe() == "builtin"){
					...
					EditorGUILayout.LabelField("Please switch to URP or HDRP render pipeline to work with Clayxels <3", s);
		...
		void inspectMaterial(ClayContainer clayContainer){
			// #if UNITY_EDITOR_OSX
				// we can't use unity's builtin material editor Os X M1 macs, it just doesn't work
				this.drawCustomInspector(clayContainer);
			// #else
			// 	EditorGUI.BeginChangeCheck();
		...
		void drawCustomInspector(ClayContainer container){
			EditorGUILayout.Space();
